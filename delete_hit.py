...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Delete an existing HIT.')

    parser.add_argument('--sandbox', dest='sandbox', action='store_true', help='Whether to use the sandbox')
    parser.add_argument('--hit_id', dest='hit_id', help='HIT ID of HIT to delete', required=True)

    args = parser.parse_args()

    client = get_client(sandbox=args.sandbox)
    delete_hit(client, args.hit_id)
